...
void pcl_cb(sensor_msgs::PointCloud2::ConstPtr cloud_msg){
  ...
  if (!(newCloud->width < min_points_)) { //newcloud->points.size()
    ...
    //i never wrote this!
    if (max_iter_ == 3)
      if ( (theta[0]<theta[1] && theta[0]>theta[2]) || (theta[0]>theta[1] && theta[0]<theta[2]) )
	avgTheta = theta[0];
      else if ( (theta[1]<theta[0] && theta[1]>theta[2]) || (theta[1]>theta[0] && theta[1]<theta[2]) )
	avgTheta = theta[1];
      else
	avgTheta = theta[2];
...
int main(int argc, char **argv) {
  ...
  serviceOff = nh.advertiseService("/scan_registration/switchOffRansac", turnOff); 
  ...
  theta_pub_ = nh.advertise<geometry_msgs::Pose2D>("/scan_registration/tarDir",1);
