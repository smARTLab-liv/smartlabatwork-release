...
from slaw_srvs.srv import setRegistrationTar, switchOff
from slaw_actions.msg import RegistrationFineAdjustAction, RegistrationFineAdjustGoal, RegistrationFineAdjustResult
...
class fakeMonitor(object):
    ...
    def sendPauseRansac(self):
        rospy.wait_for_service('/scan_registration/switchOffRansac')
        try:
            switch_off = rospy.ServiceProxy('/scan_registration/switchOffRansac', switchOff)
    ...
    def processKey(self, key):
        ...
        if key == 'pause\n':
            ...
            self.sendPause()
        if key == 'ransac\n':
            print "(un)pausing ransac.."
            self.sendPauseRansac()
        if key == 'test\n':
            print "action test"
